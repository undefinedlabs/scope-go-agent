...
func (test *Test) end() {
	...
	reason, source := test.extractTestLoggerOutputWithFailOrSkipReason(test.t.Failed() || test.t.Skipped())
...
func (test *Test) extractTestLoggerOutputWithFailOrSkipReason(failedOrSkipped bool) (reason, source string) {
	...
	if len(matches) > 0 && failedOrSkipped {
		fsIdx := len(matches) - 1
		fsMatch := matches[fsIdx]
		matches = matches[:fsIdx]
		reason = fsMatch[3]
		source = fmt.Sprintf("%s:%s", fsMatch[1], fsMatch[2])
	} else {
		reason = ""
		source = ""
	}
